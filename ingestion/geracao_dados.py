...
def write_csv(filename, rows, fieldnames, delimiter=","):
    path = os.path.join(OUTPUT_DIR, filename)
    with open(path, "w", newline="", encoding="utf-8") as f:
        w = csv.DictWriter(f, fieldnames=fieldnames, delimiter=delimiter)
        w.writeheader()
        w.writerows(rows)
    print(f"  OK {filename} ({len(rows)} linhas)")
...
coletas_base = []
coleta_id = 1
fazendas_ativas = [f for f in fazendas if True]
...
for _ in range(200):
    coletas_base.append(random.choice(coletas_base[:5000]).copy())

coletas_fonte_a = []
coletas_fonte_b = []
coletas_fonte_c = []

for row in coletas_base:
    destino = random.choice(["a", "b", "c"])

    if destino == "a":
        coletas_fonte_a.append({
            "id_coleta": row["id_coleta"],
            "id_fazenda": row["id_fazenda"],
            "id_rota": row["id_rota"],
            "data_coleta": row["data_coleta"].strftime("%d/%m/%Y"),
            "volume_litros": row["volume_litros"],
            "temperatura_c": row["temperatura_c"],
            "gordura_pct": row["gordura_pct"],
            "proteina_pct": row["proteina_pct"],
            "acidez": row["acidez"],
        })
    elif destino == "b":
        vol = str(row["volume_litros"]).replace(".", ",") if row["volume_litros"] != "" else ""
        temp = str(row["temperatura_c"]).replace(".", ",") if row["temperatura_c"] != "" else ""
        gordura = str(row["gordura_pct"]).replace(".", ",") if row["gordura_pct"] != "" else ""
        proteina = str(row["proteina_pct"]).replace(".", ",") if row["proteina_pct"] != "" else ""
        acidez = str(row["acidez"]).replace(".", ",") if row["acidez"] != "" else ""
        coletas_fonte_b.append({
            "collection_id": row["id_coleta"],
            "farm_id": row["id_fazenda"],
            "route_id": row["id_rota"],
            "collection_date": row["data_coleta"].strftime("%m-%d-%Y"),
            "collected_liters": vol,
            "temperature_c": temp,
            "fat_pct": gordura,
            "protein_pct": proteina,
            "acidity": acidez,
        })
    else:
        data_mista = row["data_coleta"].strftime("%Y-%m-%d") if random.random() < 0.5 else row["data_coleta"].strftime("%d/%m/%Y")
        coletas_fonte_c.append({
            "COLETA ID": row["id_coleta"],
            "ID FAZENDA": row["id_fazenda"],
            "ID ROTA": row["id_rota"],
            "DATA": data_mista,
            "VOLUME": row["volume_litros"],
            "TEMPERATURA": row["temperatura_c"],
            "GORDURA": row["gordura_pct"],
            "PROTEINA": row["proteina_pct"],
            "ACIDEZ": row["acidez"],
            "OBSERVACAO": random.choice(["", "Digitado manual", "Revisar valor"]),
        })

write_csv("coletas_fonte_a.csv", coletas_fonte_a,
          ["id_coleta", "id_fazenda", "id_rota", "data_coleta", "volume_litros",
           "temperatura_c", "gordura_pct", "proteina_pct", "acidez"], delimiter=";")
write_csv("coletas_fonte_b.csv", coletas_fonte_b,
          ["collection_id", "farm_id", "route_id", "collection_date", "collected_liters",
           "temperature_c", "fat_pct", "protein_pct", "acidity"])
write_csv("coletas_fonte_c.csv", coletas_fonte_c,
          ["COLETA ID", "ID FAZENDA", "ID ROTA", "DATA", "VOLUME",
           "TEMPERATURA", "GORDURA", "PROTEINA", "ACIDEZ", "OBSERVACAO"])
...
# ─── sumário de problemas injetados ──────────────────────────────────────────
print("\nResumo dos problemas de qualidade injetados intencionalmente:")
print("-" * 90)
problemas = [
    ("Datas em formato DD/MM/YYYY",       "~4% dos registros com datas",           "normalize_date() no ingestao_bronze.py"),
    ("Valores nulos em campos opcionais", "~5-10% por coluna nullable",            "NULLIF() nos models Silver"),
    ("Duplicatas de registros",           "5 cooperados, 200 coletas, 50 pedidos", "ROW_NUMBER() + _is_duplicate flag"),
    ("Volume de leite negativo",          "~1% das coletas",                       "CASE WHEN volume < 0 THEN NULL no stg_coletas"),
    ("Preço de produto zerado",           "~3% dos produtos",                      "NULLIF(preco, 0) no stg_produtos"),
    ("Preço de item negativo",            "~0.5% dos itens_pedido",                "CASE WHEN preco < 0 THEN NULL no stg_itens_pedido"),
    ("Capitalização inconsistente",       "~25-30% dos campos categóricos",        "UPPER(TRIM()) nos models Silver"),
]
print(f"  {'Problema':<42} {'Onde':<42} {'Tratamento'}")
print("-" * 90)
for p, onde, trat in problemas:
    print(f"  - {p:<40} {onde:<42} -> {trat}")
print()

print("Todos os arquivos gerados em /raw_data/")
print("   Incluindo coletas em múltiplos formatos: coletas_fonte_a.csv, coletas_fonte_b.csv, coletas_fonte_c.csv")
